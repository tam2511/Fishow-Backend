      ...
      <div class="card-content">
        ...
        <div class="media card-footer">
          <div class="is-flex blog-author">
            ...
            <p>{{ report.author }}</p>
            <time datetime="2016-1-1">{{ report.created_at }}</time>
          </div>
          <div>
            <b-field grouped group-multiline>
              <div class="control">
                <b-taglist attached>
                  <nuxt-link
                    :to="{ name: 'report-slug', params: { slug: report.slug } }"
                  >
                    <b-tag type="is-light">Комментариев</b-tag>
                    <b-tag type="is-primary">{{ report.comments_count }}</b-tag>
                  </nuxt-link>
                </b-taglist>
              </div>
            </b-field>
...
export default {
  ...
  data() {
    return {
      result: {},
      userLikedBlog: this.report && this.report.user_has_votedUp,
      userDisLikedBlog: this.report && this.report.user_has_votedDown,
      likesCounter: 0,
      dislikesCounter: 0,
    }
  },
  head() {
    return {
      script: [
        {
          innerHTML: this.createJsonBlog(this.report),
  ...
  computed: {
    getResult() {
      try {
        return JSON.parse(this.report.content).blocks[0]
      } catch (e) {
        return null
      }
    },
    rating() {
      return this.report.likes_count - this.report.dislikes_count
    },
  },
  ...
  methods: {
    createJsonBlog(data) {
      ...
      const images = []
      try {
        const items = JSON.parse(this.report.content).blocks[0]
    ...
    likeBlog() {
      ...
      this.$axios.$post(`/report/${this.report.id}/like/`)
    },
    unLikeBlog() {
      this.likesCounter -= 1
      this.userLikedBlog = false
      this.$axios.$delete(`/report/${this.report.id}/like/`)
    },
    dislikeBlog() {
      this.dislikesCounter -= 1
      this.userDisLikedBlog = true
      this.$axios.$post(`/report/${this.report.id}/dislike/`)
    },
    undislikeBlog() {
      this.dislikesCounter += 1
      this.userDisLikedBlog = false
      this.$axios.$delete(`/report/${this.report.id}/dislike/`)
    },
  },
}
</script>
