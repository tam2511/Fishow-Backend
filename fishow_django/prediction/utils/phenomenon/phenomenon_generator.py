...
class PhenomenonTextGenerator:
    ...
    def get_day_desc(data, date, fish):
        ...
        day_data = sum([(_.temperature, _.phenomenon) for _ in data if _.date == date and _.fish == fish], [])
        filtred_data = sum([(_.temperature, _.phenomenon) for _ in data if _.date in observe_dates and _.fish == fish],
                           [])
